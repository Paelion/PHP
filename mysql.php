...
$pdo = new PDO('mysql:host=127.0.0.1;port=8889;dbname=php', "root", "root");
...
$cp->execute();
$cp = $pdo->prepare("CREATE TABLE type(
        ID INT PRIMARY KEY AUTO_INCREMENT,
        NAME VARCHAR(255),
        bonus INT(11)
        )");
...
$perf = $pdo->prepare('SELECT NAME,ATK FROM personnages');
$perf->execute();
$stat = $perf->fetchAll(PDO::FETCH_OBJ);
if (!empty($stat)):
...
## UTILISER L'OBJ PDO POUR RECUPERER LES INFORMATIONS DE LA BASE DE DONNEE
$perf2 = $pdo->prepare('SELECT * FROM personnages ORDER BY ID DESC');
$perf2->execute();
$stat2 = $perf2->fetchAll(PDO::FETCH_OBJ);
if (!empty($stat2)):
...
$typeF = $pdo->prepare('SELECT * FROM type');
$typeF->execute();
$type = $typeF->fetchAll(PDO::FETCH_OBJ);
if (!empty($type)):
...
<?php
echo "------------------- EXO 12 ------------------- <br> <br>";## REPRENDRE LE FORMULAIRE FAIT AU DESSUS, AU NIVEAU DU SELECTEUR FAIRE UN FOREACH POUR AFFICHER DANS LE SELECTEUR TOUT LES TYPES DISPONIBLE DANS LA BASE DE DONNEES
; ?>
...
if (!empty($_POST)) {
    ...
    $atk = $_POST["atk"];
    $type = $_POST["type"];
    $add_form = $pdo->prepare('INSERT INTO personnages (NAME,ATK,type_id) VALUES ("' . $name . '",' . $atk . ', (SELECT ID FROM type WHERE name LIKE "' . $type . '"))');
    $add_form->execute();
}
